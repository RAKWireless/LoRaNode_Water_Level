...
static int handle_join_mode(lora_config_t *config, int argc, char *argv[], char *in, cfg_op op);
...
static int write_config_string(lora_config_t *config, const char *in)
{
    int i;
    int ret;
    int argc;
...
static int handle_join_mode(lora_config_t *config, int argc, char *argv[], char *in, cfg_op op)
{
    ...
    if (op == CFG_READ) {
        ...
        len += sprintf(buffer + len, "%d", config->join_mode);
        ret = len;
    } else {
        if (argc != 1) {
            goto END;
        ...
        config->join_mode = atoi(argv[0]);
        if (config->join_mode < MAX_MODE) {
           ret = RAK_OK;
        }
    }
END:
