...
void rw_ReadUsrConfig(void)
{
  ...
  DPRINTF("sizeof(g_lora_config)= %d, default Region_%s\r\n", sizeof(g_lora_config), rw_Region2Str(LORAMAC_REGION_DEFAULT));
  e_printf("Default Region_%s\r\n",  rw_Region2Str(LORAMAC_REGION_DEFAULT));
  ret = read_partition(PARTITION_0, (char *)&g_lora_config, sizeof(g_lora_config));
  if (ret >= 0) {
    if ( rw_Str2Region(g_lora_config.region) == LORAMAC_REGION_DEFAULT) {
       return ;
    }
  }
...
#if 0
  memcpy(g_lora_config.dev_addr, &DevAddr, 4);
  memcpy(g_lora_config.nwks_key, &NwkSKey, 16);
  memcpy(g_lora_config.apps_key, &AppSKey, 16);
  
  e_printf("version: 1.0.2\r\n");
  
  e_printf("Dev_EUI: ");
  dump_hex2str(DevEui , 8);
  e_printf("DevAddr: %08X\r\n", DevAddr);
  e_printf("NwkSKey: ");
...
#if 1 
  g_lora_config.ch_mask[0] =0x0f00;
  g_lora_config.ch_mask[1] =0x0000;
  g_lora_config.ch_mask[2] =0x0000;
  g_lora_config.ch_mask[3] =0x0000;
  g_lora_config.ch_mask[4] =0x0000;
...
//  mibReq.Type = MIB_CHANNELS_DATARATE;
//  mibReq.Param.ChannelsDatarate = 1; 
//  LoRaMacMibSetRequestConfirm( &mibReq );
