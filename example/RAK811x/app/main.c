...
int32_t main (void)
{
	...
	////SYS_SetChipClockSrc((CLK_PWRCTL_HIRC_EN | CLK_PWRCTL_LXT_EN), 1);
	CLK->PLLCTL=0x00031300; //disable PLL,   PLL set FOUT= 12M;
	
	CLK->PWRCTL= CLK_PWRCTL_HIRC_EN;
	while(SYS_CheckChipClockSrc( CLK_CLKSTATUS_HIRC_STB ) != 0) ;
	
	#if((WDT_WAKEUP_TEST==1)	|| (TIMER0_WAKEUP_TEST==1))
	CLK->PWRCTL |=  CLK_PWRCTL_LIRC_EN;
...
#if(GPIOA_WAKEUP_TEST==1)			
GPIOA->IMD=0; //GPIOA ¡À???¡ä£¤¡¤¡é
GPIOA->IER=0xffffffff; //??¡À???¡ä£¤¡¤¡é
NVIC_EnableIRQ(GPABC_IRQn);
#endif
		
#if(WDT_WAKEUP_TEST==1)		
	    //Enable WDT clock 
		UNLOCKREG(); 
	SYS_SetWDTClock(1);  

	// Enable WDT and interrupt 
	WDT->CTL = WDT_CTL_WTIS_2POW16 | WDT_CTL_WTWKE | WDT_CTL_WTE;
...
#if(TIMER0_WAKEUP_TEST==1)	
                        CLK->CLKSEL1=(CLK->CLKSEL1&0xfffff8ff)| 0x00000700; //select HIRC for Timer0
                        // Timer clk = (10KHz / (prescale + 1 = 10)) = 1KHz
                        TIMER_Init(TIMER0, 9, 1000, TIMER_CTL_MODESEL_PERIODIC|TIMER_CTL_WAKEEN);
                       /* Enable TIMER0 Intettupt */
                        TIMER_EnableInt(TIMER0, TIMER_IER_TMRIE);
												CLK->CLKSEL1=(CLK->CLKSEL1&0xfffff8ff)| 0x00000200; //select LIRC for Timer0
                        TIMER_Start(TIMER0);

#endif
 
	
#if(NORMAL_RUN_TEST==1)			
	while(1){ __NOP();}  //¡¤??a?a¨°????¨¦¨°?2a??DD¡Á¡ä¨¬?¦Ì?¦Ì?¨¢¡Â
#else
	 GPIOD->DOUT=0x01;
